...
|---|---|---|
| **Typo: `discrimnator`** (missing `i`)  | Low | Field name is preserved in `DatasetRecipe` and serialized JSON for backward compatibility. New code must use the misspelled name. |
| **Typo: `injestion`** (should be `ingestion`) | Low | Used throughout file paths, JSON field names, and log messages. A rename would break existing Bronze file path parsing. |
...
| **Parquet → DuckDB migration in progress** | Medium | `silver_data_contracts.md` and some code still references Parquet-based Silver storage. The keymap-driven DuckDB path is the target; the old Parquet path should be treated as legacy. |
| **No pagination support** | Medium | The `DatasetRecipe` contract explicitly defers pagination. Large datasets (e.g., price history for thousands of tickers) rely on `from_date` windowing rather than offset/cursor pagination, which may miss data if a single window exceeds API result limits. |
