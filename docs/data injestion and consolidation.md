...
✅ **As a Data Engineer**, I want to fetch quarterly financials, dividends, and share counts from Alpha Vantage so that raw data for all tickers is available as JSON.

**Acceptance**: Raw API responses are persisted as JSON files in the data lake for a sample list of tickers.
Bronze retains raw API responses as JSON, while Silver/Gold use Parquet.

**Recommended JSON Folder Structure (partitioned by symbol):**
...
- Each partition contains one or more JSON files for that symbol and endpoint.
