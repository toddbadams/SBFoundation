...
class DashboardApp:
    def __init__(self):
        ...
        self.logger = factory.create_logger(self.__class__.__name__)
        ...
        self.screener_page = ScreenerPage(
            ...
            self.unique_year_quarters,
            self.selected_year_quarter,
            self.logger,
        )
        self.dividend_page = DividendScorePage(
            self.config_loader,
            self.overview_df,
            self.consolidated_df,
            self.tickers,
    ...
    def main(self):
        # page config
        st.set_page_config(page_title="Strawberry", layout="wide", initial_sidebar_state="expanded", page_icon=":strawberry:")
        ...
        pages: dict[str, callable] = {"Screener": self.screener_page.render, "Dividend Score": self.dividend_page.render}
