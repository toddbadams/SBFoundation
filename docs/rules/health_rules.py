...
class HealthRules:
    ...
    def __init__(self, earnings_to_fcf_lower_threshold: float = 0.4, earnings_to_fcf_upper_threshold: float = 0.6, peg_upper_threshold: float = 1.0):
    ...
    def cash_conversion_rule(self, df: pd.DataFrame) -> pd.DataFrame:
        df["rule_cash_conversion"] = (df["ebitda_to_fcflow"] >= self.earnings_to_fcf_lower_threshold) & (
