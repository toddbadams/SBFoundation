...
def _get_git_sha(self) -> str:
    return TestData.GOLD_BUILD_MODEL_VERSION
...
def _test_setup(port: str, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    data_root_path = Path(tempfile.mkdtemp(prefix="strawberry-e2e-", dir=repo_root_path)).resolve()
    ...
    monkeypatch.setattr(GoldService, "_get_git_sha", _get_git_sha)
...
def test_01_data_layer_promotion_full_run(fake_api_server: tuple[str, FakeApiServer], monkeypatch: pytest.MonkeyPatch):
    ...
    with bootstrap.connect() as conn:
        ...
        assert_query_records(conn, TestData.MarketCap.SQL_GOLD_FACT, TestData.MarketCap.GOLD_EXPECTED, TestData.MarketCap.GOLD_DATE_FIELDS)
        assert_query_records(conn, TestData.Dates.SQL_GOLD_DIM_DATE, TestData.Dates.GOLD_EXPECTED_DIM_DATE, TestData.Dates.GOLD_DATE_FIELDS)
        assert_query_records(
            conn, TestData.CompanyProfile.SQL_GOLD_DIM, TestData.CompanyProfile.GOLD_EXPECTED_DIM, TestData.CompanyProfile.GOLD_DATE_FIELDS
        )
        assert_ops_records(conn, TestData.Ops.SQL_OPS_FILE_INGESTIONS, TestData.Ops.OPS_EXPECTED_FILE_INGESTIONS, TestData.Ops.OPS_DATE_FIELDS)
...
def test_02_orchestrator_stage_sequence(fake_api_server: tuple[str, FakeApiServer], monkeypatch: pytest.MonkeyPatch):
    ...
    with bootstrap.connect() as conn:
        ...
        assert_empty(conn, TestData.MarketCap.SQL_GOLD_FACT)
        assert_empty(conn, TestData.Dates.SQL_GOLD_DIM_DATE)
        assert_empty(conn, TestData.CompanyProfile.SQL_GOLD_DIM)
    ...
    bootstrap = DuckDbBootstrap()
    with bootstrap.connect() as conn:
        assert_query_records(conn, TestData.MarketCap.SQL_GOLD_FACT, TestData.MarketCap.GOLD_EXPECTED, date_fields=["date"])
        assert_query_records(conn, TestData.Dates.SQL_GOLD_DIM_DATE, TestData.Dates.GOLD_EXPECTED_DIM_DATE, date_fields=["date"])
        assert_query_records(
            conn, TestData.CompanyProfile.SQL_GOLD_DIM, TestData.CompanyProfile.GOLD_EXPECTED_DIM, TestData.CompanyProfile.GOLD_DATE_FIELDS
        )
...
def test_03_instrument_discovery_flow(fake_api_server: tuple[str, FakeApiServer], monkeypatch: pytest.MonkeyPatch):
    ...
    4. Gold dim_instrument table is populated with SCD2 tracking
    """
    port, _ = fake_api_server
    _test_setup(port, monkeypatch)
    ...
    Orchestrator(
        OrchestrationSettings(
            ...
            enable_bronze=True,
            enable_silver=True,
            enable_gold=True,  # Enable gold to test dim_instrument
            enable_non_ticker_run=True,
    ...
    stock_list_bronze = get_most_recent_bronze_path(
        TestData.StockList.DOMAIN, TestData.StockList.SOURCE, TestData.StockList.DATASET
    )
    assert stock_list_bronze is not None, "stock-list Bronze file not created"
    assert stock_list_bronze.is_file()

    etf_list_bronze = get_most_recent_bronze_path(
        TestData.ETFList.DOMAIN, TestData.ETFList.SOURCE, TestData.ETFList.DATASET
    )
    ...
    with bootstrap.connect() as conn:
        ...
        # Verify Gold dim_instrument table is populated with SCD2 tracking
        dim_instrument_df = conn.execute('SELECT * FROM "gold"."dim_instrument"').fetchdf()
        assert len(dim_instrument_df) == expected_total, f"Expected {expected_total} dim_instrument rows, got {len(dim_instrument_df)}"

        # Verify SCD2 columns are present and correct
        assert "is_current" in dim_instrument_df.columns, "is_current column missing from dim_instrument"
        assert "effective_from" in dim_instrument_df.columns, "effective_from column missing from dim_instrument"
        assert "effective_to" in dim_instrument_df.columns, "effective_to column missing from dim_instrument"

        # All records should be current (is_current = True) on initial load
        current_records = dim_instrument_df[dim_instrument_df["is_current"] == True]
        assert len(current_records) == expected_total, f"Expected all {expected_total} records to be current"

        # Verify specific instruments exist in Gold dimension
        aapl_gold = dim_instrument_df[dim_instrument_df["symbol"] == "AAPL"]
        assert len(aapl_gold) == 1, "AAPL not found in dim_instrument"
        assert aapl_gold.iloc[0]["is_current"] == True
        assert aapl_gold.iloc[0]["instrument_type"] == "equity"

        spy_gold = dim_instrument_df[dim_instrument_df["symbol"] == "SPY"]
        assert len(spy_gold) == 1, "SPY not found in dim_instrument"
        assert spy_gold.iloc[0]["is_current"] == True
        assert spy_gold.iloc[0]["instrument_type"] == "etf"
...
def assert_ops_records(conn: duckdb.DuckDBPyConnection, sql: str, expected: dict, date_fields: list[str]) -> None:
    rel = conn.execute(sql)
    arrow = rel.fetch_arrow_table()
    df = arrow.to_pandas(date_as_object=True)
    ...
    # Normalize date fields -> ISO-8601 "YYYY-MM-DD"
    for field in date_fields:
        if field in actual:
            actual[field] = _to_iso_date(actual[field])
